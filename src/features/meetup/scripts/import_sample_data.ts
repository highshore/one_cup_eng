...
export const importSampleMeetupData = async (): Promise<void> => {
  try {
    ...
    console.log(`📊 Total events created: ${Object.keys(sampleMeetupData).length}`);
...
export const checkExistingData = async (): Promise<boolean> => {
  try {
    ...
    const docSnapshot = await import('firebase/firestore').then(({ getDoc }) => getDoc(firstDocRef));
...
export const runImport = async (force: boolean = false): Promise<void> => {
  try {
    if (!force) {
      ...
      if (hasExistingData) {
        console.log('⚠️  Sample data already exists. Use force=true to overwrite.');
...
if (typeof window !== 'undefined') {
  ...
  console.log('💡 You can run importMeetupData() in the console to import sample data');
}

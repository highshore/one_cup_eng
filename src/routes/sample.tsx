...
const highlightWithHyphens = (text: string): string => {
  // Split text into words, preserving punctuation and whitespace
  ...
  return parts
    .map((part) => {
        ...
        return subParts
          .map((subPart) => {
            ...
            const highlightCount = Math.max(
              1,
              Math.min(3, Math.floor(subPart.length / 2))
            );
            const highlighted = subPart.slice(0, highlightCount);
            const rest = subPart.slice(highlightCount);
      ...
      const highlightCount = Math.max(
        1,
        Math.min(5, Math.floor(part.length / 2))
      );
...
const FloatingButton = styled.a<{ isVisible: boolean }>`
  position: fixed;
  bottom: 50px; // Position above the 70px audio player
  left: 50%;
  transform: translateX(-50%);
  width: calc(100% - 3rem); // Match container padding (1.5rem on each side)
  max-width: 850px; // Match container max-width
  box-sizing: border-box;
  background-color: ${colors.primary};
  color: white;
  padding: 15px 20px;
  ...
  transform: translateX(-50%)
    translateY(${(props) => (props.isVisible ? "0" : "20px")}); // Add slide-in effect
  ...
  @media (max-width: 880px) {
    // Adjust breakpoint slightly larger than max-width to account for padding
    width: calc(100% - 2rem); // Match mobile container padding (1rem each side)
  }

  @media (max-width: 768px) {
    padding: 10px 16px;
    font-size: 0.9rem;
    bottom: 75px; // Adjust bottom for mobile audio player space
  }
`;

const SampleArticlePage = () => {
  const hardcodedArticleId = "MPbTLIGCWGEzAYCOO4V0"; // Hardcoded ID
        ...
        if (isAudioMode && article?.audio?.characters) {
          ...
          if (
            ...
            article.content.english.forEach((paragraph, paragraphIndex) => {
              const words = paragraph.split(/\s+/); // Corrected regex
          ...
          if (
            ...
            const words =
              article.content.english[currentParagraphIndex].split(/\s+/); // Corrected regex
            if (words.length > 0) {
  ...
  useEffect(() => {
    const fetchArticle = async () => {
      // Use the hardcoded ID directly
      const articleIdToFetch = hardcodedArticleId;
      if (!articleIdToFetch) {
      ...
      try {
        const articleRef = doc(db, "articles", articleIdToFetch);
    ...
    fetchArticle();
  }, [hardcodedArticleId]); // Dependency is now the hardcoded ID constant
  ...
  const calculateReadingTime = (content: string[]): string => {
    const totalWords = content.reduce((acc, paragraph) => {
      return acc + paragraph.split(/\s+/).length; // Corrected regex
    }, 0);
  ...
  const handleWordClick = async (e: React.MouseEvent) => {
    ...
    if (
      ...
      (selectedWord.split(/\s+/).length > 1 && !selectedWord.includes("-")) // Corrected regex
    ) {
    ...
    // Escape the selected word for use in regex
    const escapedWord = selectedWord.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const sentenceRegex = new RegExp(
      `[^.!?]*\\b${escapedWord}\\b[^.!?]*[.!?]`, // Use the escaped word here
      "i"
    );
    ...
    try {
      ...
      // Use hardcoded ID for fetching definition
      const articleIdForDefinition = hardcodedArticleId;
      if (!articleIdForDefinition) {
        throw new Error("Article ID is missing for definition lookup");
      }
      const definition = await getWordDefinition(
        selectedWord,
        context,
        articleIdForDefinition
      );
  ...
  useEffect(() => {
    ...
    article.content.english.forEach((paragraph, paragraphIndex) => {
      ...
      const words = paragraph.split(/(\s+)/); // Keep this regex for positioning
      let wordStart = 0;
      let wordIndex = 0;
  ...
  return (
    <ArticlePageWrapper isAudioMode={isAudioMode}>
      ...
      {/* Render FloatingButton conditionally based on isAudioMode */}
      <FloatingButton href="/profile" isVisible={!isAudioMode}>
        👉🏻 매일 영어 한잔 받아보기 👈🏻
      </FloatingButton>
    </ArticlePageWrapper>
  );

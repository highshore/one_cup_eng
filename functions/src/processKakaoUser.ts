  ...
  async (request: CallableRequest<ProcessKakaoUserRequestData>) => {
    if (!request.auth || request.auth.uid !== request.data.oidcUserUid) {
      throw new HttpsError(
        "unauthenticated",
        "The function must be called by the authenticated OIDC user."
      );
    }
    ...
    if (!kakaoAccessToken || typeof kakaoAccessToken !== "string") {
      throw new HttpsError(
        "invalid-argument",
        "Missing or invalid 'kakaoAccessToken' string."
      );
    }
    if (!kakaoSub || typeof kakaoSub !== "string") {
      throw new HttpsError(
        "invalid-argument",
        "Missing or invalid 'kakaoSub' string."
      );
    }
    if (!oidcUserUid || typeof oidcUserUid !== "string") {
      throw new HttpsError(
        "invalid-argument",
        "Missing or invalid 'oidcUserUid' string."
      );
    }

    logger.log(
      `processKakaoUser: Started for OIDC UID: ${oidcUserUid}, Kakao Sub: ${kakaoSub}`
    );
    ...
    try {
      ...
      logger.log(
        "Kakao raw user info for OIDC UID",
        oidcUserUid,
        JSON.stringify(kakaoUserInfo)
      );
    } catch (error: any) {
      logger.error(
        "Error fetching Kakao user info for OIDC UID",
        oidcUserUid,
        error.response?.data || error.message
      );
      throw new HttpsError(
        "internal",
        `Failed to fetch Kakao user info for ${oidcUserUid}.`
      );
    }

    const kakaoPhoneNumberRaw = kakaoUserInfo.kakao_account?.phone_number;
    const kakaoEmail = kakaoUserInfo.kakao_account?.email;
    let kakaoProfileImageUrl =
    ...
    if (kakaoProfileImageUrl && kakaoProfileImageUrl.startsWith("http://")) {
      kakaoProfileImageUrl = kakaoProfileImageUrl.replace(
        "http://",
        "https://"
      );
    }
    ...
    if (normalizedKakaoPhone) {
      try {
        const existingUserByPhone = await admin
          .auth()
          .getUserByPhoneNumber(normalizedKakaoPhone);
        logger.log(
          `Found existing Firebase Auth user by phone. Matched UID: ${existingUserByPhone.uid}, Phone: ${existingUserByPhone.phoneNumber}`
        );

        if (existingUserByPhone.uid !== oidcUserUid) {
          logger.log(
            `Phone-matched UID ${existingUserByPhone.uid} differs from OIDC UID ${oidcUserUid}. Merging Kakao into phone account.`
          );

          // Delete the temporary OIDC user
          await admin
            .auth()
            .deleteUser(oidcUserUid)
            .catch((err) =>
              logger.error(
                `Failed to delete OIDC user ${oidcUserUid} during phone merge:`,
                err
              )
            );
          ...
          if (kakaoProfileNickname && !existingUserByPhone.displayName)
            updatePayloadForPhoneUser.displayName = kakaoProfileNickname;
          if (kakaoEmail && !existingUserByPhone.email)
            updatePayloadForPhoneUser.email = kakaoEmail; // Consider email verification implications
          if (kakaoProfileImageUrl && !existingUserByPhone.photoURL)
            updatePayloadForPhoneUser.photoURL = kakaoProfileImageUrl;

          if (Object.keys(updatePayloadForPhoneUser).length > 0) {
            await admin
              .auth()
              .updateUser(existingUserByPhone.uid, updatePayloadForPhoneUser);
            logger.log(
              `Updated Auth for phone-matched user ${existingUserByPhone.uid} with Kakao details (only if empty):`,
              JSON.stringify(updatePayloadForPhoneUser)
            );
          }

          const existingUserDocRef = admin
            .firestore()
            .collection("users")
            .doc(existingUserByPhone.uid);
          ...
          logger.log(
            `Updated Firestore for phone-matched user ${existingUserByPhone.uid} with kakaoId: ${kakaoSub}.`
          );

          const customToken = await admin
            .auth()
            .createCustomToken(existingUserByPhone.uid);
          return {
            status: "merged_by_phone",
            customToken: customToken,
            finalUid: existingUserByPhone.uid,
            message:
              "Account merged: Kakao identity linked to existing phone-verified user.",
          };
        }
        ...
        logger.log(
          `Phone number ${normalizedKakaoPhone} is already associated with OIDC user ${oidcUserUid}. No merge needed based on phone.`
        );
      } catch (error: any) {
        if (error.code === "auth/user-not-found") {
          logger.log(
            `No Firebase user found with phone '${normalizedKakaoPhone}'. Proceeding to check by kakaoSub.`
          );
        } else {
          logger.error(
            `Error in getUserByPhoneNumber for phone '${normalizedKakaoPhone}', OIDC UID ${oidcUserUid}:`,
            error
          );
    ...
    if (!querySnapshot.empty) {
      ...
      logger.log(
        `Found existing Firestore user by kakaoId ${kakaoSub}. Matched UID: ${firestoreUserUid}`
      );

      if (firestoreUserUid !== oidcUserUid) {
        logger.log(
          `Firestore-matched UID ${firestoreUserUid} differs from OIDC UID ${oidcUserUid}. Merging Kakao into Firestore account.`
        );
        ...
        await admin
          ...
          .catch((err) =>
            logger.error(
              `Failed to delete OIDC user ${oidcUserUid} during kakaoId merge:`,
              err
            )
          );
        logger.log(`Deleted temporary Firebase OIDC user ${oidcUserUid}.`);
        ...
        if (kakaoProfileNickname && !firestoreAuthUser.displayName)
          updatePayloadForFirestoreUser.displayName = kakaoProfileNickname;
        if (kakaoEmail && !firestoreAuthUser.email)
          updatePayloadForFirestoreUser.email = kakaoEmail;
        if (kakaoProfileImageUrl && !firestoreAuthUser.photoURL)
          updatePayloadForFirestoreUser.photoURL = kakaoProfileImageUrl;
        if (normalizedKakaoPhone && !firestoreAuthUser.phoneNumber)
          updatePayloadForFirestoreUser.phoneNumber = normalizedKakaoPhone;

        if (Object.keys(updatePayloadForFirestoreUser).length > 0) {
          await admin
            .auth()
            .updateUser(firestoreUserUid, updatePayloadForFirestoreUser);
          logger.log(
            `Updated Auth for Firestore-matched user ${firestoreUserUid} with Kakao details (only if empty/new):`,
            JSON.stringify(updatePayloadForFirestoreUser)
          );
        }
        ...
        const customToken = await admin
          .auth()
          .createCustomToken(firestoreUserUid);
        return {
          status: "merged_by_kakao_id",
          customToken: customToken,
          finalUid: firestoreUserUid,
          message:
            "Account merged: Kakao identity linked to existing user found by kakaoId.",
        };
      }
      // If firestoreUserUid IS oidcUserUid, it means OIDC user already linked via kakaoId. Proceed to P3 for updates.
      logger.log(
        `kakaoId ${kakaoSub} is already associated with OIDC user ${oidcUserUid}. No merge needed based on kakaoId.`
      );
    }
    ...
    logger.log(
      `Proceeding to finalize Auth and Firestore for OIDC UID: ${oidcUserUid} with Kakao Sub: ${kakaoSub}`
    );
    ...
    if (kakaoProfileNickname && !finalAuthUser.displayName)
      authUpdatePayload.displayName = kakaoProfileNickname;
    if (kakaoEmail && !finalAuthUser.email)
      authUpdatePayload.email = kakaoEmail; // Consider email verification needs
    if (kakaoProfileImageUrl && !finalAuthUser.photoURL)
    ...
    if (normalizedKakaoPhone && !finalAuthUser.phoneNumber) {
      ...
      logger.log(
        `Preparing to add phone ${normalizedKakaoPhone} to Auth user ${oidcUserUid}`
      );
    }
    ...
    if (Object.keys(authUpdatePayload).length > 0) {
      try {
        ...
        logger.log(
          `Successfully updated Firebase Auth for OIDC UID ${oidcUserUid} (only if empty/new phone):`,
          JSON.stringify(authUpdatePayload)
        );
      } catch (authError: any) {
        logger.error(
          `Error updating Firebase Auth for OIDC UID ${oidcUserUid}:`,
          authError,
          "Payload:",
          JSON.stringify(authUpdatePayload)
        );
    ...
    const firestoreUserData: any = {
      ...
      lastLoginAt: admin.firestore.FieldValue.serverTimestamp(),
      displayName:
        updatedFinalAuthUser.displayName || kakaoProfileNickname || null,
    ...
    if (!userDocSnapshot.exists) {
      logger.log(
        `Creating NEW Firestore document for OIDC UID: ${oidcUserUid}`
      );
      ...
      logger.log(
        `Updating EXISTING Firestore document for OIDC UID: ${oidcUserUid}`
      );
      // Ensure phone field in Firestore is consistent with Auth, if it changed.
      const existingData = userDocSnapshot.data();
      if (
        updatedFinalAuthUser.phoneNumber &&
        (!existingData ||
          existingData.phone !== updatedFinalAuthUser.phoneNumber)
      ) {
    ...
    logger.log(
      `Successfully created/updated Firestore document for OIDC UID ${oidcUserUid}.`
    );
    ...
    return {
      ...
      message:
        "Kakao login processed. User Auth/Firestore updated for OIDC user.",
    };
  }
);

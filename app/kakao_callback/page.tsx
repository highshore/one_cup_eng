...
interface ProcessKakaoUserResponse {
  status:
    ...
    | "merged_by_kakao_id"
    | "success_oidc_user"
    | string; // Allow other strings for robustness
...
const KakaoCallbackContent = () => {
  ...
  const processKakaoUser = httpsCallable<
    ProcessKakaoUserRequestData,
    ProcessKakaoUserResponse
  >(functions, "processKakaoUser");
    ...
    const processLogin = async (authCode: string) => {
      try {
        ...
        if (
          ...
          console.error(
            "Kakao callback: Failed to get tokens from Kakao.",
            tokenData
          );
          router.push("/");
          return;
        ...
        try {
          ...
          router.push("/auth"); // Critical error, redirect to auth
          return;
        }
        ...
        console.log(
          "Temporarily signed in with Kakao OIDC. UID:",
          oidcUser.uid
        );

        console.log(
          "Calling processKakaoUser function with Kakao Access Token, Sub, and OIDC UID..."
        );

        const processResult: HttpsCallableResult<ProcessKakaoUserResponse> =
          await processKakaoUser({
            kakaoAccessToken,
            kakaoSub,
        ...
        if (
          (functionData.status === "merged_by_phone" ||
            functionData.status === "merged_by_kakao_id") &&
          functionData.customToken &&
          functionData.finalUid
        ) {
          console.log(
            `Account merged (status: ${functionData.status}). Current OIDC UID: ${oidcUser.uid}, Final UID to switch to: ${functionData.finalUid}`
          );
          if (oidcUser.uid !== functionData.finalUid) {
            // Check if a switch is actually needed
            await signOut(auth);
            console.log("Signed out temporary Kakao OIDC user.");
            await signInWithCustomToken(auth, functionData.customToken);
            console.log(
              "Signed in with custom token for merged user:",
              functionData.finalUid
            );
          } else {
            ...
            console.log(
              "OIDC user UID is the same as final UID. Re-signing with custom token (if provided) to ensure claims or if merge logic resulted in this."
            );
            await signOut(auth).catch((e) =>
              console.warn(
                "Sign out failed before custom token re-sign (same user), might be okay:",
                e
              )
            ); // Attempt sign out just in case
            await signInWithCustomToken(auth, functionData.customToken);
          }
          ...
          if (returnUrl) {
            console.log(
              "[KakaoCallback] Redirecting to stored return URL:",
              returnUrl
            );
            ...
            console.log(
              "[KakaoCallback] No return URL found, redirecting to profile"
            );
            router.push("/profile");
          }
        } else if (
          functionData.status === "success_oidc_user" &&
          functionData.finalUid === oidcUser.uid
        ) {
          // This case means the OIDC user was processed (new or existing OIDC user updated)
          console.log(
            "processKakaoUser successful for OIDC user (Kakao linked & Firestore doc created/updated). UID:",
            functionData.finalUid
          );
          ...
          console.error(
            "processKakaoUser returned an unexpected status or missing data:",
            functionData
          );
          ...
          if (auth.currentUser) {
            ...
            await signOut(auth).catch((err) =>
              console.warn(
                "Sign out after failed processKakaoUser failed:",
                err
              )
            );
          }
        ...
        if (auth.currentUser) {
          ...
          await signOut(auth).catch((signOutError) =>
            console.warn("Sign out during error handling failed:", signOutError)
          );
        }
        router.push("/auth");
      }
